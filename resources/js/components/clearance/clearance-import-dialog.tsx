...
/**
 * Backed by the queued ProcessClearanceImport job — same polling flow
 * as EmployeeFileImportDialog/BonaVidaImportDialog. CSV-only (the
 * source sheet is a flat single-table CSV export, no XLSX variant).
 * Insert-only, so there's no "updated" count to show.
 */
export default function ClearanceImportDialog({ open, onOpenChange }: Props) {
        ...
        const pollStatus = async () => {
            try {
                const response = await fetch(
                    `/import/clearance/${importStatus.id}/status`,
    ...
    function submit(event: FormEvent) {
        ...
        router.post('/import/clearance', data, {
            forceFormData: true,

            onSuccess: (page) => {
                const flash = (page.props as any)?.flash ?? page.props;

                const raw = flash?.success ?? 'Clearance import started.';
                ...
                if (Number.isInteger(importId) && importId > 0) {
                    // total_rows is null until the queue worker has read the file.
                    setImportStatus({
                        id: importId,
                        status: 'pending',
                        total_rows: null,
    ...
    function cancelImport() {
        ...
        router.post(
            `/import/clearance/${importStatus.id}/cancel`,
            {},
            {
                onFinish: () => {
                    setCancelling(false);
    ...
    useEffect(() => {
        ...
        if (importStatus.status === 'completed') {
            setSummary({
                message: `Import complete — ${importStatus.created_rows.toLocaleString()} record(s) created.`,
                skipped:
            ...
            // New offices/types can be auto-created by the import, so
            // refresh the filter options along with the table.
            router.reload({ only: ['records', 'statuses', 'forms', 'offices'] });
        }
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle>Import Clearance Records</DialogTitle>

                    <DialogDescription>
                        Upload the Clearance monitoring CSV. Colleges/offices
                        are matched by name and created automatically if
                        they don't already exist — this always adds new
                        records, it does not update existing ones.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <DialogFooter className="flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <a
                            href="/import/template/clearance"
