...
export default function OfficesImportDialog({ open, onOpenChange }: Props) {
    ...
    */
    ...
    | Poll import status
    |--------------------------------------------------------------------------
    | Lives on the component instance, not on dialog visibility, so polling
    | keeps running even while the dialog is closed (as long as the parent
    | keeps this component mounted). That's what lets you close the modal
    | mid-import and see progress again when you reopen it.
    */
        ...
        const pollStatus = async () => {
            try {
                const response = await fetch(
                    `/import/offices/${importStatus.id}/status`,
                    {
                        headers: {
    ...
    | Open / close
    |--------------------------------------------------------------------------
    | Closing the dialog (X, backdrop click, Esc) does not reset state.
    ...
    */
    ...
    | Submit
    |--------------------------------------------------------------------------
    | CSV uploads are queued (ProcessOfficesImport). The redirect's flash
    | "success" message carries "<message>|||import_id:<id>" so we can pick
    | up polling immediately, same as RRSP/RRPPE/WMR/Bona-Vida.
    */
    ...
    function submit(event: FormEvent) {
        ...
        data.append('merge_existing', '1');

        router.post('/import/offices', data, {
            forceFormData: true,

            onSuccess: (page) => {
                const flash = (page.props as any)?.flash ?? page.props;

                const raw = flash?.success ?? 'Offices import started.';
    ...
    | While an import is running: asks the backend to cancel the job via
    |   POST /import/offices/{import}/cancel
    | which marks the import row 'cancelled' (ProcessOfficesImport checks
    | that flag between progress checkpoints and bails out early).
    */
    ...
    function cancelImport() {
        ...
        router.post(
            `/import/offices/${importStatus.id}/cancel`,
            {},
            {
                onFinish: () => {
                    setCancelling(false);
    ...
    */
    ...
    useEffect(() => {
        ...
        if (importStatus.status === 'completed') {
            setSummary({
                message: `Import complete — ${importStatus.processed_rows.toLocaleString()} offices processed.`,
                skipped:
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Upload your Offices CSV. Existing office codes are
                        updated in place; new office codes are created.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <div>
                        ...
                        {!file ? (
                            <div
                                role="button"
                                tabIndex={submitting || isImporting ? -1 : 0}
                                onClick={() => {
                                    if (!submitting && !isImporting) {
                            ...
                            >
                                ...
                                <p className="mt-3 text-[11px] text-muted-foreground">
                                    Use the supplied template to ensure the
                    ...
                    <DialogFooter className="flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button
                                type="submit"
                                disabled={submitting || isImporting || !file}
