...
/**
 * Backed by the queued ProcessEmployeeFileImport job — this mirrors
 * BonaVidaImportDialog's polling flow. CSV-only (xlsx uploads are no
 * longer accepted). Note there's no "updated" count here (the import
 * is insert-only), so that stat is just never shown.
 */
export default function EmployeeFileImportDialog({ open, onOpenChange }: Props) {
    ...
    function isCsv(selectedFile: File): boolean {
        const name = selectedFile.name.toLowerCase();

        return selectedFile.type === 'text/csv' || name.endsWith('.csv');
    }
        ...
        const pollStatus = async () => {
            try {
                const response = await fetch(
                    `/import/employee-files/${importStatus.id}/status`,
    ...
    function submit(event: FormEvent) {
        ...
        router.post('/import/employee-files', data, {
            forceFormData: true,

            onSuccess: (page) => {
                const flash = (page.props as any)?.flash ?? page.props;

                const raw = flash?.success ?? 'Employee File Locator import started.';
    ...
    function cancelImport() {
        ...
        router.post(
            `/import/employee-files/${importStatus.id}/cancel`,
            {},
            {
                onFinish: () => {
                    setCancelling(false);
    ...
    useEffect(() => {
        ...
        if (importStatus.status === 'completed') {
            ...
            router.reload({ only: ['records', 'statuses'] });
        }
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle>Import Employee File Locator</DialogTitle>

                    <DialogDescription>
                        Upload the Employee File Locator CSV. Active
                        and Dead file entries are both read from the sheet
                        and imported as new records — this always adds new
                        rows, it does not update existing ones.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <DialogFooter className="flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <a
                            href="/import/template/employee-files"
