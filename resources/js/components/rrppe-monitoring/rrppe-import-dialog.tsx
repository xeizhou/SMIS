...
export default function RrppeImportDialog({ open, onOpenChange }: Props) {
        ...
        const pollStatus = async () => {
            try {
                const response = await fetch(
                    `/import/rrppe/${importStatus.id}/status`,
    ...
    function submit(event: FormEvent) {
        ...
        router.post('/import/rrppe', data, {
            forceFormData: true,

            onSuccess: (page) => {
                const flash = (page.props as any)?.flash ?? page.props;

                const raw = flash?.success ?? 'RRPPE import started.';
    ...
    function cancelImport() {
        ...
        router.post(
            `/import/rrppe/${importStatus.id}/cancel`,
            {},
            {
                onFinish: () => {
                    setCancelling(false);
    ...
    useEffect(() => {
        ...
        if (importStatus.status === 'completed') {
            setSummary({
                message: `Import complete — ${importStatus.processed_rows.toLocaleString()} RRPPEs processed.`,
                skipped:
                    importStatus.skipped_rows > 0
                        ? [
                              `${importStatus.skipped_rows.toLocaleString()} RRPPEs skipped.`,
                          ]
                        : [],
            });
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle>Import RRPPE Records</DialogTitle>

                    <DialogDescription>
                        Upload your RRPPE monitoring CSV. Rows with a blank
                        RRPPE no. are added as extra items of the RRPPE above
                        them.
                    </DialogDescription>
                </DialogHeader>
