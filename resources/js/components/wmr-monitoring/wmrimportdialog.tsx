...
export default function WmrImportDialog({ open, onOpenChange }: Props) {
        ...
        const pollStatus = async () => {
            try {
                const response = await fetch(
                    `/import/wmr/${importStatus.id}/status`,
    ...
    function submit(event: FormEvent) {
        ...
        router.post('/import/wmr', data, {
            forceFormData: true,

            onSuccess: (page) => {
                const flash = (page.props as any)?.flash ?? page.props;

                const raw = flash?.success ?? 'WMR import started.';
    ...
    function cancelImport() {
        ...
        router.post(
            `/import/wmr/${importStatus.id}/cancel`,
            {},
            {
                onFinish: () => {
                    setCancelling(false);
    ...
    useEffect(() => {
        ...
        if (importStatus.status === 'completed') {
            setSummary({
                message: `Import complete — ${importStatus.processed_rows.toLocaleString()} WMRs processed.`,
                skipped:
                    importStatus.skipped_rows > 0
                        ? [
                              `${importStatus.skipped_rows.toLocaleString()} WMRs skipped.`,
                          ]
                        : [],
            });
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle>Import WMR Records</DialogTitle>

                    <DialogDescription>
                        Upload your WMR monitoring CSV. Each WMR is a block of
                        rows that starts with the supplier's name, the same
                        layout as the WMR logbook.
                    </DialogDescription>
                </DialogHeader>
