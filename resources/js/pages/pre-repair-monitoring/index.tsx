...
export default function Index({ data = { data: [], links: [], current_page: 1, last_page: 1, per_page: 10, total: 0, from: null, to: null }, filters = {}, itrPtrs = [] }: { data?: PaginatedPreRepair, filters?: any, itrPtrs?: ITRPTRMonitoring[] }) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/pre-repair-monitoring', { search: searchQuery, condition_of_ppe: conditionFilter === 'all' ? undefined : conditionFilter }, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                            {data.data.length > 0 ? (
                                data.data.map((item) => (
                                    <tr 
                                        key={item.id} 
                                        className={'transition-colors duration-1000 ' + 'hover:bg-gray-50/50 dark:hover:bg-gray-800/50'} data-search-0={item.property_no} data-search-1={item.pre_repair_no} data-record-id={item.id}
                                    >
